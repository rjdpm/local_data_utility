...
FUNCTION:
-----------------------------------------------------------------------------
@AUTHOR: Soumitra Samanta                DATE: Tue Dec  1 23:35:12 2020
...
class SmIm2Sm_Network():
    ...
    #         network = nn.DataParallel(network)
        ...
        # # intermediate results save path
        # self.result_savepath = ''.join([result_savepath, 'vae_network_', self.layer_type, '_nkernels_', str(self.num_kernels).replace(' ','_').strip('[]').replace(',',''), '_szkernel_', str(self.size_kernels).replace(' ','_').strip('[]').replace(',',''), '_nfc_encoder_', str(self.num_fc_layer_encoder), '_nlatent_dims_', str(self.dims_reduce), '_szgruhidden_', str(self.gru_hidden_size), '_ngru_', str(self.num_gru), '_w_inint_', self.weight_init, '_loss_', self.loss_type, '_solver_', self.solver_type, '_nepoch_', str(self.num_epoch), '_bsize_', str(self.batch_size), '_lr_', str(self.learning_rate), '/']).replace('.','_')
        
        # self.result_savepath = create_folder(self.result_savepath)
        # # to save best network    
        # self.best_network_save_filename = ''.join(['best_network_', self.result_savepath.split('/')[-2]]).replace('.','_')        
    #------------------------------------------------------
    ...
    def train_1epoch(self) -> float:
        ...
        for i in tqdm(range(num_iteration)):# iteration over each minibatch
            ...
            loss, kld_loss, bce_loss = self.loss_function(_x, _x_bar, _mu, _sigma)# loss calculation
