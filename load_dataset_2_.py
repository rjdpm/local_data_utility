...
class Mol2VecDataset:
    ...
    def normalize_data(self, X_train, X_val, X_test, y_train, y_val, y_test,
        ...
        with open(f'{self.mol2vec_dataset_path}/StandardScaler_features.pkl', 'wb') as fp:
    ...
    def prepare_datasets(self,
        ...
        X_train, X_val, X_test, y_train, y_val, y_test= self.normalize_data(X_train.values, X_val.values, X_test.values,
...
class DescriptorsDataset:
    ...
    def preprocess_features(self, X_train, X_val,
                            ...
                            mi_threshold=0.05, corr_threshold=0.95, non_corr_threshold=0.05, force=False):
        """
        ...
        if preprocessing:
            if os.path.isfile(optimal_feature_path) and (not force):
                print(f'Optimal features file exists. \nLoading optimal features from: {optimal_feature_path}')
                with open(optimal_feature_path, 'r') as f:
                    optimal_features = json.load(f)
            else:
                print(f'Optimal features file not exists. \nCreating optimal features list: {optimal_feature_path}')
                print('-'*80)
                print('='*80)
                # Mutual info selection
                print('Selecting features with most Mutual Information:')
                selected_features = self._select_by_mutual_info(X_train=X_train, y_train=y_train, threshold=mi_threshold)
                X_train = X_train[selected_features]
                print(f'Number of columns after removing one of the highly correlated features: {len(X_train.columns)}')
                print('-'*80)
                ...
                print('Removing non-correlated features with target values')
                ...
                print(f'Number of columns after removing non-correlated features: {len(optimal_features)}')
                print('-'*80)
                
                # Optional feature selection
                if feature_selection:
                    print('Selecting features in hierarchical order')
                    optimal_features = self._apply_hierarchical_fs(X_train, y_train, X_val, y_val)
                    print(f'Number of columns after hierarchical features selection: {len(optimal_features)}')
                    print('-'*80)
...
class ChemBertDataset:
    ...
    def find_max_length(self,
                        max_len_path = f'./max_length_smiles.pkl'
                        ):
    ...
    def create_dataset(self, data_augmentation=False):
        ...
        # all_dataset.set_format("torch", columns=["input_ids", "attention_mask", "labels"])
